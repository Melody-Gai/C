...
// ######################################################################## //
//
//  本程序使用链式表实现二叉树的建立, 遍历与操作.
//
//  创建日期: 2017-04-16
//
//  修改记录:
//  ( 1 ) 2017-04-16 创建了初始版本( v0.10 )
//  ( 2 ) 2017-04-25 改进了版本( v0.15 )
//
//  版 本 号: v0.15
//
//  备    注: 使用标准 C 代码实现
...
int CalcBinaryTreeDegree( BiTreePtr Root )
{
    ...
    n0Count = CountBinaryTreeLeafNodeNum(Root);
    n2Count = n0Count - 1;
...
int DeleteBinaryTreeNode( BiTreePtr Root, char NodeData )
{
  ...
  if (p) {
      q = FindBinaryTreeNodeFather(Root, NodeData, &ChildType);
      q->LChild == p ? q->LChild = NULL : q->RChild = NULL;
      DestroyBinaryTree(p);
      p = NULL;
  }
  else {
...
void DisplayBinaryTreeNodeAncestors( BiTreePtr Root, char NodeData )
{
    ...
    while (q != Root) {
        q = FindBinaryTreeNodeFather(Root, p->Data, &ChildType);
        str[count++] = q->Data;
        p = q;
    }
    printf("\t祖先节点为：");
