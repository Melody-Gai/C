...
char MSG_BASE_CHAR_SET[6] = "ABCDE";
...
void HuffmanDecoding( char *EncodingMsg, char *OrigMsg, int NumOfCharSet, HuffmanTreeNode *HuffmanTree, HuffmanCodeNode *HuffmanCodeBook )
{
    int k = 0;
    int p = 0;
    int Root = NumOfCharSet * 2 - 2;
    int len = strlen(OrigMsg);
    while (k < len) {
        Root = NumOfCharSet * 2 - 2;
        while ((HuffmanTree+Root)->LChild != -1) {
            if (*(EncodingMsg + p) == '0') {
        ...
        *(OrigMsg + k) = (HuffmanCodeBook + Root)->BaseChar;
        k++;
    }
