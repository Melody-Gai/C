...
// IndexingSearch.cpp : 定义控制台应用程序的入口点。
//
...
KeyPosRecord IndexSearch( int Key, int *pData, BlockIndexRecord *pBlock, int RecNum, int BlockNum )
{
  ...
  for (k = 0; k < BlockNum; k++) {
      if ((pBlock + k)->MaxKey >= Key) {
          break;
      }
  }

  KeyPos.BlockNo = k + 1;
  RecNumPerBlock = SeqSearch(Key, pData + k*6, 6);
  KeyPos.Pos = RecNumPerBlock;
...
void main( void )
{
  ...
  DataPtr  = ( int * )malloc( sizeof( int ) * ( MAX_RECORD_NUM + MAX_BLOCK_NUM ) );
  IndexPtr = ( BlockIndexRecord * )malloc( sizeof( BlockIndexRecord ) * MAX_BLOCK_NUM );
